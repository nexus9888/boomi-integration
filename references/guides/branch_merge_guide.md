...
**Opt-in feature.** Branch and merge is not enabled on all accounts and not all users choose to use it. Do not use branch-related tools or suggest branching workflows unless the user has explicitly indicated they work with branches (via session instructions, project configuration files like OPENCODE.md, AGENTS.md, or CLAUDE.md, or direct request).
